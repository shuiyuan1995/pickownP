...
class CreateOutPacketsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('out_packets', function (Blueprint $table) {
            ...
            $table->integer('seed_sum')->comment('发送总额');
            $table->integer('number')->comment('中奖数字');
            $table->integer('surplus_sum')->comment('剩余总额');
            $table->integer('count')->comment('红包总个数');
            $table->integer('up')->comment('单个红包金额上限，整数，5表示为500');
            $table->integer('down')->comment('单个红包金额下限，存储为整数，如5表示为500');
            $table->integer('surplus_count')->comment('剩余个数');
            $table->integer('status')->comment('状态(1-未领完，2-已领完，3-退回，4-冻结)');
